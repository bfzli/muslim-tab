...
import type { SettingsProps, SearchProvider, GradientType } from '@types'
import { searchProviders, gradients } from '@types'
import { loadGoogleFont } from '@services'
...
const Settings: React.FC<SettingsProps> = ({
    ...
    useEffect(() => {
        if (highlightedIndex >= 0 && fontDropdownRef.current) {
            const dropdown =
                fontDropdownRef.current.querySelector('[data-dropdown]')
            const highlightedOption = dropdown?.children[
                highlightedIndex
            ] as HTMLElement
            if (highlightedOption && dropdown) {
                ...
                if (optionTop < scrollTop) {
                    ...
                    dropdownEl.scrollTop =
                        optionBottom - dropdownEl.clientHeight
                }
            }
        }
    }, [highlightedIndex])
    ...
    const handleFontChange = (fontKey: string) => {
        loadGoogleFont(fontKey, allFonts)
            .then(() => {
    ...
    return createPortal(
        ...
        >
            ...
            <ModalBody ref={modalBodyRef}>
                ...
                {showSearch && (
                    ...
                    >
                        ...
                        <ProviderSelector>
                            {Object.entries(searchProviders).map(
                                ([key, config]) => (
                                    <ProviderButton
                                        key={key}
                    ...
                    <FontSelectorWrapper ref={fontDropdownRef}>
                        ...
                        <FontDropdown isOpen={isFontDropdownOpen} data-dropdown>
                            ...
                            {fontEntries.length === 0 ? (
                                <FontNotFound>
                                    No fonts found for "
                                    {fontSearch.length > 18
                                        ? fontSearch.slice(0, 18) + '...'
                                        : fontSearch}
                                    "
                                </FontNotFound>
                            ) : (
            ...
            <SettingsFooter>
                ...
                <FooterLink
                    href='https://github.com/bfzli/muslim-tab'
                    target='_blank'
                    rel='noopener noreferrer'
                >
                    <svg
